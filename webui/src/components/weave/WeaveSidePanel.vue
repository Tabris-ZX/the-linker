<template>
  <aside class="weave-side-panel" aria-label="织链工具">
    ...
    <fieldset class="weave-panel-section">
      ...
      <div class="weave-tool-row weave-tool-row-meta" aria-label="占位与排除标记">
        <button
      ...
      <div class="weave-clear-row" aria-label="清除辅助标记">
        <button type="button" @click="app.clearWeaveUnknownMarks">清除 ? 标记</button>
        <button type="button" @click="app.clearWeaveExcludedMarks">清除 x 标记</button>
      </div>
    </fieldset>
    ...
    <fieldset class="weave-panel-section weave-feedback-section">
      ...
      <div v-if="app.weaveEndpointFeedback.length" class="weave-feedback" aria-live="polite">
        ...
        >
          {{ item.pairId }} · {{ item.isCorrect ? "Accept" : "Wrong Answer" }}
        </p>
      </div>
      <p v-else class="weave-status-text">提交后显示每个 id 的结果</p>
    </fieldset>
  </aside>
</template>
