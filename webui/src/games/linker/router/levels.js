...
export async function fetchLevelIndex() {
  const response = await fetch(withCacheBuster("/api/linker/levels/index"), {
    cache: "no-cache",
...
export async function fetchLevelDetail(levelId, sourcePath = "") {
  const url = sourcePath
    ? `/api/linker/level?path=${encodeURIComponent(sourcePath)}`
    : `/api/linker/levels/${encodeURIComponent(levelId)}`;
...
export async function fetchLevelAnswers(sourcePath = "") {
  ...
  const response = await fetch(withCacheBuster(`/api/linker/level/answers?path=${encodeURIComponent(sourcePath)}`), {
    cache: "no-cache",
    headers: getDeveloperAuthHeaders()
  });
  if (response.ok) return response.json();
 ...
 * 通过 FastAPI 后端保存关卡。
 *
 * 浏览器代码不能直接写本地文件，所以保存只能走后端 /api/linker/levels。
 *
...
export async function generateLevelRequest(payload) {
  const response = await fetch("/api/linker/levels/generate", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...getDeveloperAuthHeaders()
...
export async function checkLevelGoodRequest(payload) {
  const response = await fetch("/api/linker/levels/check-good", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...getDeveloperAuthHeaders()
...
function getLevelApiUrl() {
  return "/api/linker/levels";
}

/** 返回关卡审核 API 地址列表。 */
function getLevelReviewApiUrls() {
  return ["/api/linker/levels/review"];
}
