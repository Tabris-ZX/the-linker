import { edgeKey, getAllGridEdges, getGridBounds, getGridNodes, keyOf, lineAttrs, linePathD, pointsFromEdgeKey, toRenderPoint } from "./utils.js";
import { buildFinderHiddenEndpoints, buildFinderPairDistanceMap, buildFinderVisibleLevel } from "../../shared/utils/finderRules.js";
...
export const computed = {
    ...
    finderPenaltyText() {
      return this.formatElapsedTime(this.finderPenaltyMs);
    },
    ...
    canUseFinderMode() {
      return Boolean(this.currentLevel?.gridType === "square");
    },

    isFinderModeEnabled() {
      return this.activeView === "finder";
    },

    finderVisibleLevel() {
      return this.isFinderModeEnabled ? buildFinderVisibleLevel(this.currentLevel) : null;
    },

    finderHiddenEndpoints() {
      return this.isFinderModeEnabled ? buildFinderHiddenEndpoints(this.currentLevel) : [];
    },

    finderVisibleEndpointKeys() {
      return new Set((this.finderVisibleLevel?.pairs ?? [])
        .flatMap((pair) => pair.points ?? [])
        .map(([x, y]) => keyOf(x, y)));
    },

    finderPairDistances() {
      return buildFinderPairDistanceMap(this.finderHiddenEndpoints, this.finderKnownMarkedEndpoints);
    },

    activeBoardLevel() {
      return this.finderVisibleLevel ?? this.currentLevel;
    },
    ...
    finderModeSubtitle() {
      return "行列线索显示剩余隐藏端点数量。";
    },

    finderModeUnavailableText() {
    ...
    finderPrimaryPairOptions() {
      ...
      return this.currentLevel.pairs.map((pair) => ({
        ...
        distance: this.finderPairDistances[pair.id]?.distance ?? null,
        distanceStatus: this.finderPairDistances[pair.id]?.status ?? "empty"
      }));
    },

    finderMetaPairOptions() {
      return [
        {
          id: this.finderUnknownPairId,
          label: "?",
          color: "var(--finder-unknown-color)",
          isUnknown: true
        },
        {
          id: this.finderExcludedPairId,
          label: "×",
          color: "var(--finder-excluded-color)",
          isExcluded: true
        }
      ];
    },
    ...
    finderPairOptions() {
      return [
        ...this.finderPrimaryPairOptions,
        ...this.finderMetaPairOptions
      ];
    },

    finderKnownMarkedEndpoints() {
      const metaPairIds = new Set([this.finderUnknownPairId, this.finderExcludedPairId]);
      return Object.fromEntries(
        Object.entries(this.finderMarkedEndpoints ?? {})
          .filter(([, pairId]) => pairId && !metaPairIds.has(pairId))
      );
    },

    finderClueRows() {
      return this.buildFinderClueLines("row");
    },

    finderClueColumns() {
      return this.buildFinderClueLines("column");
    },

    finderHiddenEndpointCount() {
      return this.finderHiddenEndpoints.length;
    },

    finderMarkedEndpointCount() {
      return Object.keys(this.finderKnownMarkedEndpoints).length;
    },
    ...
    levelDirectoryStatusText() {
      ...
      if (!this.isDeveloperMode) return `关卡 ${counts.stable}`;
      return `关卡 ${counts.total} · 正式版 ${counts.stable} · 测试版 ${counts.alpha} · 待删版 ${counts.removed}`;
    },
    ...
    boardNodes() {
      ...
      for (const [x, y] of getGridNodes(this.activeBoardLevel)) {
        ...
        const finderMarkPairId = this.finderMarkedEndpoints[key] ?? "";
        const isUnknownFinderMark = finderMarkPairId === this.finderUnknownPairId;
        const isExcludedFinderMark = finderMarkPairId === this.finderExcludedPairId;
        const finderMarkPoint = isUnknownFinderMark || isExcludedFinderMark
          ? {
              id: finderMarkPairId,
              label: isExcludedFinderMark ? "×" : "?",
              color: isExcludedFinderMark ? "var(--finder-excluded-color)" : "var(--finder-unknown-color)"
            }
          : finderMarkPairId ? this.getPair(finderMarkPairId) : null;
        const renderedEndpoint = endpoint;
        const renderedFinderMark = !renderedEndpoint && finderMarkPoint ? finderMarkPoint : null;
        const shouldRenderPathNode = requiredNodes.has(key);
        if (!endpoint && !shouldRenderPathNode && !finderMarkPairId) continue;
        const [renderX, renderY] = this.toBoardDisplayPoint([x, y]);
        nodes.push({
          key,
          x,
          ...
          classes: {
            ...
            "finder-hidden-endpoint": false,
            "finder-hidden-target": false,
            "finder-marked-node": Boolean(finderMarkPairId),
            "finder-mark-unknown": isUnknownFinderMark,
            "finder-mark-excluded": isExcludedFinderMark
          }
        });
      }
