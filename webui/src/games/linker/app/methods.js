...
export const methods = {
    ...
    async toggleWeaveMode(enabled) {
      if (enabled && !this.canEnterWeaveMode()) {
        this.weaveStatusText = this.weaveModeUnavailableText || "数寻暂不可用";
        return;
      }
      if (!enabled) {
        this.clearWeaveAttemptState();
    ...
    async selectGameMode(mode) {
      if (mode === "weave" || mode === "weave-total" || mode === "weave-id") {
        if (mode === "weave-id") this.weaveStatusText = "数寻当前使用总数线索";
        await this.toggleWeaveMode(true);
        if (!this.weaveStatusText || this.weaveStatusText === "已进入数寻模式") {
    ...
    async submitWeaveEndpoints() {
      if (!this.canEnterWeaveMode()) {
        this.weaveStatusText = this.getWeaveModeUnavailableText() || "数寻暂不可用";
        return;
      }
      ...
      this.weaveStatusText = result.missingCount > 0
        ? `${this.weaveSubmitSummary}。还剩 ${result.missingCount} 个端点。罚时 ${this.weavePenaltyText}`
        : `${this.weaveSubmitSummary}。罚时 ${this.weavePenaltyText}`;
