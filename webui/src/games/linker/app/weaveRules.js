...
export function buildWeaveVisibleLevel(level) {
...
export function buildWeaveHiddenEndpoints(level) {
  return (level?.pairs ?? [])
    .map((pair) => {
...
export function buildWeavePairDistanceMap(hiddenEndpoints, markedEndpoints = {}) {
...
export function calculateWeavePenaltyMs(wrongCount, difficulty = 1) {
...
export function buildWeaveSubmissionResult(level, markedEndpoints = {}, difficulty = 1) {
  ...
  return {
    feedback: feedback.sort(compareWeaveFeedback),
    wrongCount,
    correctMarkedCount,
    missingCount,
    penaltyMs: calculateWeavePenaltyMs(wrongCount, difficulty),
    isVictory: hiddenEndpointCount > 0 && wrongCount === 0 && correctMarkedCount === hiddenEndpointCount
  };
}

function compareWeaveFeedback(left, right) {
...
export function buildWeaveClueLinesFromBuckets(targetBuckets, currentBuckets, limit) {
