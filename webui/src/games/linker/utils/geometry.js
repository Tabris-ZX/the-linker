...
/**
 * 将 SVG line 属性合并为 path d，减少大量独立线段节点。
 *
 * @param {Array<{ attrs?: { x1: number, y1: number, x2: number, y2: number } }>} lines 线段数据。
 * @returns {string} SVG path d 属性。
 */
export function linePathD(lines) {
  return lines
    .map((line) => line?.attrs)
    .filter(Boolean)
    .map(({ x1, y1, x2, y2 }) => `M ${formatPathNumber(x1)} ${formatPathNumber(y1)} L ${formatPathNumber(x2)} ${formatPathNumber(y2)}`)
    .join(" ");
}
