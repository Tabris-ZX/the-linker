import { appConfig, defaultPointPaletteId } from "./config.js";
...
import { fetchPresenceStats, reviewLevelRequest, sendPresenceHeartbeat, setDeveloperToken, verifyDeveloperToken } from "./router.js";
import { cloneLevel, hydrateLevel, hydrateLevelIndexItem, loadLevelAnswers, loadLevelDetail, loadLevelIndex } from "./services.js";
import { edgeKey, fromRenderPoint, getGridBounds, isAdjacent, keyOf, lineAttrs, positionToArray, samePoint, toRenderPoint } from "./utils.js";
import { clampNumber } from "./utils.js";
import { applyBackgroundImage, chooseRandomBackgroundImage } from "../../shared/background.js";
import { buildFinderClueLinesFromBuckets, buildFinderSubmissionResult } from "../../shared/utils/finderRules.js";

const COMPLETED_LEVELS_STORAGE_KEY = "the-finder-completed-levels";
const LAST_LEVEL_STORAGE_KEY = "the-finder-last-level-id";
const DEVELOPER_TOKEN_COOLDOWN_STORAGE_KEY = "the-finder-developer-token-cooldown-until";
const DEVELOPER_TOKEN_ATTEMPTS_STORAGE_KEY = "the-finder-developer-token-failed-attempts";
const PERSONALIZATION_STORAGE_KEY = "the-finder-personalization";
const GAME_STORAGE_KEY_PREFIX = "the-finder-";
...
const PRESENCE_CLIENT_STORAGE_KEY = "the-finder-presence-session-id";
...
export const methods = {
    /**
     * 同步页面级视图状态，供移动端布局规则区分普通棋盘和数寻工作台。
     *
     * @returns {void}
     */
    updateBodyViewState() {
      document.body.classList.toggle("is-finder-active", this.activeView === "finder");
    },
    ...
    async reviewTestLevel(level, action) {
      try {
        ...
        this.developerStatusText = action === "include" ? "已移入正式版" : "已移入待删版";
      } catch (error) {
        this.developerStatusText = error.message;
      }
    },
    ...
    /**
     * 获取通关结算用时。数寻模式会把累计罚时计入最终成绩。
     *
     * @returns {number} 结算毫秒数。
     */
    getVictoryElapsedMs() {
      return this.normalizeTimerElapsedMs(this.timerElapsedMs + this.finderPenaltyMs);
    },
    ...
    buildVictoryShareText() {
      const levelName = this.currentLevel?.name || this.currentLevel?.id || "未选择关卡";
      const levelId = this.currentLevel?.id ? ` (${this.currentLevel.id})` : "";
      const gameMode = this.isFinderModeEnabled ? "数寻" : "数链";
    ...
    async goToNextUncompletedLevel() {
      ...
      if (nextIndex < 0) {
        this.nextLevelStatusText = "All levels completed";
        return;
      }
    ...
    /**
     * 清理数寻模式当前尝试数据，保留是否开启由调用方决定。
     *
     * @returns {void}
     */
    clearFinderAttemptState() {
      this.finderMarkedEndpoints = {};
      this.finderEndpointFeedback = [];
      this.finderSubmitSummary = "";
      this.finderStatusText = "";
      this.finderPenaltyMs = 0;
      this.finderActivePairId = "";
    },
     ...
     */
    canEnterFinderMode() {
      return Boolean(this.currentLevel && this.currentLevel.gridType === "square");
    },
     ...
     * @returns {void}
     */
    async toggleFinderMode(enabled) {
      if (enabled && !this.canEnterFinderMode()) {
        this.finderStatusText = this.finderModeUnavailableText || "数寻暂不可用";
        return;
      }
      if (!enabled) {
        this.clearFinderAttemptState();
        if (this.activeView === "finder") {
          this.activeView = "finder";
        }
        return;
      }
      this.finderStatusText = "已进入数寻模式";
    },

    /**
     * 从专门入口选择游玩子模式。
     *
     * @param {"finder"} mode 模式 id。
     * @returns {Promise<void>}
     */
    async selectGameMode(mode) {
      if (mode === "finder") {
        await this.toggleFinderMode(true);
        if (!this.finderStatusText || this.finderStatusText === "已进入数寻模式") {
          this.activeView = "finder";
          this.isLevelPickerOpen = false;
          this.resetPaths();
        }
      ...
      await this.toggleFinderMode(false);
      this.resetPaths();
    },
     ...
     * @returns {string} 原因文本。
     */
    getFinderModeUnavailableText() {
      if (!this.isDeveloperMode) return "";
      if (!this.currentLevel) return "请先选择关卡";
      if (this.currentLevel.gridType !== "square") return "数寻暂只支持方形地图";
      return "";
    },
     ...
     */
    async ensureFinderAnswerCache() {
      if (!this.currentLevel?.sourcePath) return false;
      const levelKey = this.getLevelCacheKey(this.currentLevel);
      if (this.finderAnswerCacheKey === levelKey) return Array.isArray(this.currentLevel.answers) && this.currentLevel.answers.length > 0;
      if (this.isFinderAnswerLoading) return false;

      this.isFinderAnswerLoading = true;
      try {
        ...
        this.finderAnswerCacheKey = levelKey;
        return Array.isArray(answers) && answers.length > 0;
      } finally {
        this.isFinderAnswerLoading = false;
      }
    },
     ...
     */
    buildFinderHiddenEndpointBuckets(axis) {
      const buckets = new Map();
      if (!this.currentLevel || this.currentLevel.gridType !== "square") return buckets;
      this.finderHiddenEndpoints.forEach((endpoint) => {
     ...
     * @returns {Map<number, Map<string, number>>} 行/列到 id 计数。
     */
    buildFinderMarkedEndpointBuckets(axis) {
      const buckets = new Map();
      Object.entries(this.finderMarkedEndpoints ?? {}).forEach(([nodeKey, pairId]) => {
        const [x, y] = nodeKey.split(",").map(Number);
        if (!Number.isFinite(x) || !Number.isFinite(y) || !pairId) return;
        if (pairId === this.finderExcludedPairId) return;
    ...
    buildFinderClueLines(axis) {
      ...
      const targetBuckets = this.buildFinderHiddenEndpointBuckets(axis);
      const currentBuckets = this.buildFinderMarkedEndpointBuckets(axis);
      return buildFinderClueLinesFromBuckets(targetBuckets, currentBuckets, limit);
    },
     ...
     * @returns {void}
     */
    selectFinderPair(pairId) {
      this.finderActivePairId = this.finderActivePairId === pairId ? "" : pairId;
    },
     ...
     * @returns {void}
     */
    clearFinderMarksByPairId(pairId) {
      this.finderMarkedEndpoints = Object.fromEntries(
        Object.entries(this.finderMarkedEndpoints ?? {}).filter(([, markPairId]) => markPairId !== pairId)
      );
      if (this.finderActivePairId === pairId) this.finderActivePairId = "";
    },

    clearFinderUnknownMarks() {
      this.clearFinderMarksByPairId(this.finderUnknownPairId);
    },

    clearFinderExcludedMarks() {
      this.clearFinderMarksByPairId(this.finderExcludedPairId);
    },
     ...
     * @returns {void}
     */
    handleFinderNodeClick(position) {
      if (!this.isFinderModeEnabled || !this.canEnterFinderMode()) return;
      const clickedKey = keyOf(position.x, position.y);

      // 点击已有标记位置 → 直接删除（无需选中对应色点）
      if (this.finderMarkedEndpoints[clickedKey]) {
        const nextMarks = { ...this.finderMarkedEndpoints };
        delete nextMarks[clickedKey];
        this.finderMarkedEndpoints = nextMarks;
        return;
      }

      // 未选中色点 → 无法放置新标记
      if (!this.finderActivePairId) return;
      const isMetaMark = this.finderActivePairId === this.finderUnknownPairId || this.finderActivePairId === this.finderExcludedPairId;
      const pair = isMetaMark ? null : this.getPair(this.finderActivePairId);
      if (!isMetaMark && !pair) return;
      if (this.finderVisibleEndpointKeys.has(clickedKey)) return;

      // 真实色点一个 id 只能标记一个位置；问号和排除标记只是辅助标记，可以放多个。
      const nextMarks = isMetaMark
        ? { ...this.finderMarkedEndpoints }
        : Object.fromEntries(
          Object.entries(this.finderMarkedEndpoints).filter(([, pairId]) => pairId !== this.finderActivePairId)
        );
      this.finderMarkedEndpoints = {
        ...nextMarks,
        [clickedKey]: this.finderActivePairId
      };
    },
     ...
     * @returns {Promise<void>}
     */
    async submitFinderEndpoints() {
      if (!this.canEnterFinderMode()) {
        this.finderStatusText = this.getFinderModeUnavailableText() || "数寻暂不可用";
        return;
      }

      const result = buildFinderSubmissionResult(
        this.finderHiddenEndpoints,
        this.finderKnownMarkedEndpoints,
        this.normalizeLevelDifficulty(this.currentLevel?.difficulty)
      );
      this.finderEndpointFeedback = result.feedback;
      this.finderPenaltyMs += result.penaltyMs;
      this.finderSubmitSummary = result.wrongCount === 0 ? "已标记的端点均正确" : `本次提交有 ${result.wrongCount} 个错点`;
      this.finderStatusText = result.missingCount > 0
        ? `${this.finderSubmitSummary}。还剩 ${result.missingCount} 个端点。罚时 ${this.finderPenaltyText}`
        : `${this.finderSubmitSummary}。罚时 ${this.finderPenaltyText}`;
     ...
     * @returns {void}
     */
    resetFinderModeState() {
      this.clearFinderAttemptState();
      this.isFinderAnswerLoading = false;
      this.finderAnswerCacheKey = "";
    },
    ...
    handleBoardPointerDown(event) {
      ...
      if (this.isFinderModeEnabled && this.finderMarkedEndpoints[keyOf(position.x, position.y)]) {
        this.handleFinderNodeClick(position);
        event.preventDefault();
        return;
      }
    ...
    addStep(position) {
      ...
      if (mergeIndex >= 0) {
        ...
        if (!this.isFinderModeEnabled && !this.pathTouchesBothEndpoints(this.activePair, merged)) return false;
        this.paths[this.activePair] = { branches: [merged], completed: !this.isFinderModeEnabled };
        this.evaluateBoard();
        this.isDrawing = false;
        this.activePair = null;
        this.activePathMode = "";
    ...
    getCompletedPathView() {
      return Object.fromEntries(
        Object.entries(this.paths).map(([pairId]) => {
          ...
          const completed = this.isFinderModeEnabled
            ? state.branches[0]
    ...
    async revealCorrectHintLines(showEmptyStatus = true) {
      ...
      if (!hasAnswers) {
        if (showEmptyStatus) this.hintStatusText = "No answer data for this level";
        return;
      }
    ...
    evaluateBoard() {
      ...
      if (this.isFinderModeEnabled) return;
      ...
      const finderCluesSatisfied = this.isFinderPathClueSatisfied();
      const wasWon = this.isWon;
      this.isWon = allConnected && allFilled && finderCluesSatisfied;
      if (!this.isWon) {
        this.isVictoryDismissed = false;
        this.shareStatusText = "分享";
        this.nextLevelStatusText = "";
