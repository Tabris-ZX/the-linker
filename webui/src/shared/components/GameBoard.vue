  ...
  >
    ...
    <template v-if="app.currentLevel">
      ...
      >
        ...
        <span
          v-else-if="node.finderMark"
          class="dot finder-mark-dot"
          :style="app.getPointDotStyle(node.finderMark)"
        >
          <span class="dot-label" :class="{ 'is-wide': String(node.finderMark.label).length > 1 }">{{ node.finderMark.label }}</span>
        </span>
      </button>
    </template>
  </div>
