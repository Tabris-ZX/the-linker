...
const bundledBackgroundImages = Array.isArray(__BACKGROUND_IMAGES__) ? __BACKGROUND_IMAGES__ : [];
const backgroundImages = bundledBackgroundImages.length
  ? bundledBackgroundImages
  : normalizeBackgroundImages(rawConfig.background?.image ?? "background");
...
export const appConfig = {
  ...
  background: {
    path: normalizePath(rawConfig.path?.background ?? rawConfig.background?.path ?? "backgrounds"),
...
function normalizeMapStyle(config) {
  ...
  return {
    ...
    snapPointTolerance: clampNumber(Number(rawStyle.snapPointTolerance), 0.1, 0.5)
  };
}
