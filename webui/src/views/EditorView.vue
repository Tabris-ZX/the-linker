    ...
    <section class="editor-panel app-card">
      ...
      <form class="editor-form" @submit.prevent="app.writeLevelTemplate()">
        ...
        <label v-if="app.editorState.gridType !== 'equilateral-triangle'">
          宽度
          <input v-model.number="app.editorState.width" type="number" min="2" max="10" @input="app.syncEditorBounds" @wheel.prevent="app.handleEditorNumberWheel($event, 'width', app.syncEditorBounds)">
        </label>
        <label v-if="app.editorState.gridType !== 'equilateral-triangle'">
          高度
          <input v-model.number="app.editorState.height" type="number" min="2" max="10" @input="app.syncEditorBounds" @wheel.prevent="app.handleEditorNumberWheel($event, 'height', app.syncEditorBounds)">
        </label>
        <label v-if="app.editorState.gridType === 'equilateral-triangle'">
          半径
