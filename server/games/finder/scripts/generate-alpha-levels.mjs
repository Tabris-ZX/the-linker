...
function debugGenerator(message) {
  if (process.env.FINDER_GENERATOR_DEBUG) console.error(`[generator] ${message}`);
}
...
async function refreshIndexes() {
  const result = runPython("from server.games.finder.services import refresh_all_level_indexes; refresh_all_level_indexes()")
  if (result.status !== 0) {
...
function saveLevelToSqlite(level, answers) {
  const source = [
    "import json, sys",
    "from server.games.finder.services import save_level",
...
function parseArgs(args) {
  ...
  parsed.token = process.env.FINDER_DEV_TOKEN ?? "njuit1918";
  parsed.rebuildUrl = process.env.FINDER_REBUILD_URL ?? "http://127.0.0.1:8081/api/editor/finder/levels/index/rebuild";
...
function printHelp() {
  console.log(`Usage:
  node server/games/finder/scripts/generate-alpha-levels.mjs [options]
...
Examples:
  node server/games/finder/scripts/generate-alpha-levels.mjs --type square --count 3 --width 6 --height 6
  node server/games/finder/scripts/generate-alpha-levels.mjs --type right-triangle --count 3 --difficulty 4
  node server/games/finder/scripts/generate-alpha-levels.mjs --type equilateral-triangle --width 6 --height 3 --pairs 5
`);
}
