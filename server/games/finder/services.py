...
from server.games.finder.models import LevelData, LevelIndexItem, ReviewData
from server.games.finder.level_hash import (
...
def get_levels_dir() -> Path:
    """返回关卡根目录。"""
    return get_settings().finder_levels_dir
...
def get_answers_dir() -> Path:
    """返回答案文件根目录。"""
    return get_settings().finder_answers_dir
...
def read_level_index_cache() -> list[LevelIndexItem]:
    ...
    index_file = get_settings().finder_levels_index_file
