...
async function refreshIndexes() {
  const result = runPython("from server.games.linker.services import refresh_all_level_indexes; refresh_all_level_indexes()")
  if (result.status !== 0) {
...
function saveLevelToSqlite(level, answers) {
  const source = [
    "import json, sys",
    "from server.games.linker.services import save_level",
...
function parseArgs(args) {
  ...
  parsed.rebuildUrl = process.env.LINKER_REBUILD_URL ?? "http://127.0.0.1:8081/api/editor/linker/levels/index/rebuild";
...
function printHelp() {
  console.log(`Usage:
  node server/games/linker/scripts/generate-alpha-levels.mjs [options]
...
Examples:
  node server/games/linker/scripts/generate-alpha-levels.mjs --type square --count 3 --width 6 --height 6
  node server/games/linker/scripts/generate-alpha-levels.mjs --type right-triangle --count 3 --difficulty 4
  node server/games/linker/scripts/generate-alpha-levels.mjs --type equilateral-triangle --width 6 --height 3 --pairs 5
`);
}
